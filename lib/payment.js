...
class Payment {
    ...
    getById (id) {
        return new Promise((resolve, reject) => {
            var result = {};
            var strSql = `SELECT C.*, D.ID DET_ID, TARIFA, CANTIDAD1,  CANTIDAD2, UNITARIO, COTIZACION_FECHA, COTIZACION_VALOR
    ...
    checkTarifa (tarifa) {
        return new Promise((resolve, reject) => {
            ...
            var strSql = `SELECT COUNT(*) CANTIDAD
                            ...
                            WHERE TARIFA = :1`;
            this.cn.simpleExecute(strSql, [tarifa], {outFormat: this.cn.oracledb.OBJECT})
                .then(data => {
                    if (data.rows.length > 0) {
                        result = {
                            status: 'OK',
                            data: data.rows[0].CANTIDAD
                        };
                    }else {
                        result = {
                            status: 'OK',
